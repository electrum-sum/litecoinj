...
public class FakeTxBuilder {
    ...
    public static Transaction createFakeCoinbaseTx(final NetworkParameters params) {
        ...
        Transaction tx = new Transaction(params);
        tx.addInput(input);
        TransactionOutput outputToMe = new TransactionOutput(params, tx, Coin.ONEHUNDRED_COINS,
    ...
    /**
     * Transaction[0] is a feeder transaction, supplying SUM to Transaction[1]
     */
    public static Transaction[] createFakeTx(NetworkParameters params, Coin value,
                                             Address to, Address from) {
        // Create fake TXes of sufficient realism to exercise the unit tests. This transaction send SUM from the
